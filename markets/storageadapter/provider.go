...
func (n *ProviderNodeAdapter) GetAddBalanceAddress(ctx context.Context, addr address.Address, tok shared.TipSetToken) (address.Address, error) {
	return n.GetMinerWorkerAddress(ctx, addr, tok)
}
...
func (n *ProviderNodeAdapter) OnDealComplete(ctx context.Context, deal storagemarket.MinerDeal, pieceSize abi.UnpaddedPieceSize, pieceData shared.ReadSeekStarter, path string) (*storagemarket.PackingResult, error) {
	...
	for build.Clock.Since(curTime) < addPieceRetryTimeout {
		...
		if !errors.Is(err, pipeline.ErrTooManySectorsSealing) {
			if err != nil {
...
func (n *ProviderNodeAdapter) OnDealSectorPreCommitted(ctx context.Context, provider address.Address, dealID abi.DealID, proposal markettypes.DealProposal, publishCid *cid.Cid, publishHeight abi.ChainEpoch, cb storagemarket.DealSectorPreCommittedCallback) error {
	return n.scMgr.OnDealSectorPreCommitted(ctx, provider, proposal, *publishCid, cb)
}

// TODO: Remove dealID parameter, change publishCid to be cid.Cid (instead of pointer)
func (n *ProviderNodeAdapter) OnDealSectorCommitted(ctx context.Context, provider address.Address, dealID abi.DealID, sectorNumber abi.SectorNumber, proposal markettypes.DealProposal, publishCid *cid.Cid, publishHeight abi.ChainEpoch, cb storagemarket.DealSectorCommittedCallback) error {
